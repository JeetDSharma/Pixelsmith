# Copyright 2024 Thanos-DB marcoaversa
# modified from this https://github.com/Stability-AI/generative-models
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if image == None:
            ...
            noise = torch.randn(image.shape, dtype=torch.float16).to("cuda")
            ...
            while times.float().mean()>=0:          
                ...
                print(f"\r Generation process {(1-times.float().mean()/timesteps.max().item())*100:.2f}%",end="") 
                ...
                if sub_time.float().mean()>=0:
                    ...
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, sub_time.max().item())
...
def generate_image(prompt,
    ...
    if image == None:
        print("\nGenerating: ", h_res, "x", w_res)

    if image != None:
        image = image.resize((w_res, h_res), resample=PIL.Image.LANCZOS)
        print("\nGenerating: ", image.size[1], "x", image.size[0])
        image = np.array(image).astype(np.float16) / 255.0
        image = image[None].transpose(0, 3, 1, 2)
        image = torch.from_numpy(image)
        image = 2.*image - 1.
        with torch.no_grad():
            latent = pixelsmith.vae.tiled_encode(image.to("cuda"))
            image = pixelsmith.vae.config.scaling_factor * latent.latent_dist.sample()

    im = pixelsmith(prompt=prompt,
                negative_prompt=negative_prompt,
                generator=generator,
                image = image,
                slider = slider,
